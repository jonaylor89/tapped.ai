...
function LocationContent({ cityName, address, lat, lng }: Omit<LocationClientProps, "placeId">) {
  ...
  const smallVenues = useMemo(() => {
    return (venues ?? []).filter(
      (v) => v.venueInfo?.capacity && v.venueInfo.capacity < 250
    );
  }, [venues]);
  ...
  const mediumVenues = useMemo(() => {
    return (venues ?? []).filter(
      (v) =>
        (v.venueInfo?.capacity ?? 0) >= 250 &&
        (v.venueInfo?.capacity ?? 0) < 750
    );
  }, [venues]);
      ...
      <section className="mb-12">
        ...
        {performersLoading ? (
          ...
          <div className="space-y-6">
            {performersByCategory.legendary &&
              performersByCategory.legendary.length > 0 && (
                <div>
                  <h3 className="mb-3 font-semibold text-lg text-muted-foreground">
                    Legendary
                  </h3>
            ...
            {performersByCategory.mainstream &&
              performersByCategory.mainstream.length > 0 && (
                <div>
                  <h3 className="mb-3 font-semibold text-lg text-muted-foreground">
                    Mainstream
                  </h3>
            ...
            {performersByCategory.hometownHero &&
              performersByCategory.hometownHero.length > 0 && (
                <div>
                  <h3 className="mb-3 font-semibold text-lg text-muted-foreground">
                    Hometown Hero
                  </h3>
            ...
            {performersByCategory.emerging &&
              performersByCategory.emerging.length > 0 && (
                <div>
                  <h3 className="mb-3 font-semibold text-lg text-muted-foreground">
                    Emerging
                  </h3>
            ...
            {performersByCategory.undiscovered &&
              performersByCategory.undiscovered.length > 0 && (
                <div>
                  <h3 className="mb-3 font-semibold text-lg text-muted-foreground">
                    Undiscovered
                  </h3>
