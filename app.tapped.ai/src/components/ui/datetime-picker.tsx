...
function CalendarGrid({ state, ...props }: CalendarGridProps) {
	...
	const weeksInMonth = getWeeksInMonth(
		state.visibleRange.start as unknown as WeeksInMonthDate,
		locale
	);
				...
				{[...new Array(weeksInMonth).keys()].map((weekIndex) => (
					<tr className="mt-2 flex w-full" key={weekIndex}>
						{state
							...
							.map((date, i) =>
								date ? (
									<CalendarCell key={i} state={state} date={date as CalendarCellDate} />
								) : (
									<td key={i} />
								)
							)}
...
>(({ jsDate, onJsDateChange, showClearButton = true, ...props }, ref) => {
	...
	return (
		...
		>
			...
			<div className={cn("mr-2 -ml-2 h-5 w-5", !showClearButton && "hidden")}>
				<X
