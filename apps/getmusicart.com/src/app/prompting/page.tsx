...
export default function Prompting() {
  ...
  const pollInference = useCallback(
    async ({ inferenceId, modelId }: { inferenceId: string; modelId: string }) => {
      for (let i = 0; i < MAX_REPLIES; i++) {
        ...
        if (json.status === "finished") {
          ...
          router.push(`/selection?image_uris=${imageUris.join(",")}`);
          return;
        }
  ...
  useEffect(() => {
    ...
    startPolling();
  }, [inferenceId, modelId, pollInference]);

  const redirectWithInference = ({ id, modelId }: { id: string; modelId: string }) => {
  ...
  return (
    <div className="min-h-screen px-4">
      ...
      <div className="min-h-screen flex flex-col items-center justify-end md:justify-center">
        ...
        <h1 className="text-5xl tracking-tighter pb-10 font-bold text-center">describe your cover art</h1>
        <div className="md:block h-6" />
        <form className="flex w-full sm:w-auto flex-col sm:flex-row mb-10" onSubmit={submitForm}>
          <input
        ...
        <CreditsChip className="hidden md:block text-center" />
      </div>
      <div className="md:block h-6" />
    </div>
  );
