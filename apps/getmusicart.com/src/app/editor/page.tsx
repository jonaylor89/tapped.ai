...
export default function Editor() {
  ...
  if (imageUri === undefined || imageUri === null) {
    return (
      <div className="min-h-screen flex justify-center items-center">
        <h1 className="text-center text-5xl tracking-tighter font-extrabold gray-800">no image url provided</h1>
      </div>
    );
  }
  ...
  const imageLoader = ({ src, width, quality }: { src: string; width: number; quality?: number }) => {
    return `${src}?w=${width}&q=${quality || 75}&image_uri=${encodeURIComponent(imageUri)}&explicit_content=${
      explicitContent
    }`;
  };
      ...
      <div className="flex flex-col justify-center items-center md:w-1/3">
        ...
        <form>
          <div className="inline-flex items-center">
            ...
            <label className="mt-px font-light text-gray-300 cursor-pointer select-none" htmlFor="ripple-on">
        ...
        <Link
          ...
          href={`/download?image_uri=${encodeURIComponent(
            `${editorUri}?image_uri=${encodeURIComponent(imageUri)}&explicit_content=${explicitContent}`,
          )}`}
