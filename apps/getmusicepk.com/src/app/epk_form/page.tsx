...
export default function EpkForm() {
  ...
  if (user?.instagramHandle === undefined || user?.instagramHandle === null || user?.instagramHandle === "") {
    pages.splice(5, 0, IgField);
  }

  if (user?.tiktokHandle === undefined || user?.tiktokHandle === null || user?.tiktokHandle === "") {
    pages.splice(6, 0, TiktokField);
  }

  if (user?.twitterHandle === undefined || user?.twitterHandle === null || user?.twitterHandle === "") {
  ...
  return (
    <div
      className={"flex min-h-screen flex-col items-center justify-center px-4 md:px-8 lg:px-16"}
            ...
            return (
              <>
                ...
                <div className="flex justify-between mt-4 md:mt-8 lg:mt-16">
                  <button className="tapped_btn_rounded" onClick={handlePreviousPage}>
                    back
                  </button>

                  {isValid && currentIndex !== pages.length - 1 && (
                    <button className="tapped_btn_rounded_black" onClick={handleNextPage} disabled={!isValid}>
                      next
                    </button>
                  )}
                </div>
