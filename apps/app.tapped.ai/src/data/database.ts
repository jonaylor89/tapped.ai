...
import { type LabelApplication, labelApplicationConverter } from "@/domain/types/label_application";
import { type Opportunity, opportunityConverter } from "@/domain/types/opportunity";
import type { Option } from "@/domain/types/option";
import type { RecursivePartial } from "@/domain/types/recursive_partial";
...
export async function getBookingLeaders(): Promise<UserModel[]> {
	...
	const leaders = (await Promise.all(bookingLeaders.map(getUserByUsername))).filter(
		(u) => u !== null
	) as UserModel[];
...
export async function getServiceById({
	...
	const docRef = doc(db, `/services/${userId}/userServices/${serviceId}`).withConverter(
		serviceConverter
	);
	const docSnap = await getDoc(docRef);
	if (!docSnap.exists()) {
...
export async function getActiveProducts(): Promise<
	{ product: { id: string } & DocumentData; prices: ({ id: string } & DocumentData)[] }[]
> {
