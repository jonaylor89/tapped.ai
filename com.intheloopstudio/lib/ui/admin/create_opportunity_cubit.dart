...
class CreateOpportunityCubit extends Cubit<CreateOpportunityState> {
  ...
  Future<void> handleImageFromGallery() async {
    try {
      final imageFile = await state.picker.pickImage(
        source: ImageSource.gallery,
      );
      if (imageFile != null) {
        emit(state.copyWith(pickedPhoto: Option.of(File(imageFile.path))));
      }
    } catch (e) {
      // print(error);
    }
  ...
  void updateStartTime(DateTime startTime) {
    emit(state.copyWith(endTime: EndTime.dirty(startTime.add(state.duration))));

    emit(state.copyWith(startTime: StartTime.dirty(startTime)));
  }

  void updateEndTime(DateTime endTime) {
    emit(state.copyWith(endTime: EndTime.dirty(endTime)));
  }

  void onLocationChanged(Option<PlaceData> placeData, String placeId) {
    emit(state.copyWith(placeData: placeData, placeId: Option.of(placeId)));
  }

  void updateVenue(Option<UserModel> venue) {
  ...
  Future<Option<Opportunity>> submit() async {
    ...
    try {
      ...
      final op = Opportunity(
        ...
        location: location,
        venueId: state.venue.map((t) => t.id),
