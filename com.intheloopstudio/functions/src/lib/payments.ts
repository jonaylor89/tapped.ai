...
export const _createPaymentIntent = async (data: {
  ...
  if (data.destination === undefined || data.destination === null || data.destination === "") {
    throw new functions.https.HttpsError("invalid-argument", "The function argument 'destination' cannot be empty");
  }
  ...
  const customerId =
    data.customerId === undefined || data.customerId === null || data.customerId === ""
...
export const _createStripeAccount = async ({ countryCode }: { countryCode?: string }): Promise<string> => {
  const stripe = new Stripe(stripeKey.value(), {
    apiVersion: "2022-11-15",
  });
...
export const _createConnectedAccount = async ({
  accountId,
  country,
}: {
...
export const _getAccountById = async (data: { accountId: string }): Promise<Stripe.Response<Stripe.Account>> => {
  if (data.accountId === undefined || data.accountId === null || data.accountId === "") {
    throw new functions.https.HttpsError("invalid-argument", "The function argument 'accountId' cannot be empty");
  }
