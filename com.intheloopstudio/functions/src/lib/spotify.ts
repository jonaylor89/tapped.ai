...
async function getSpotifyAccessToken({ clientId, clientSecret }: { clientId: string; clientSecret: string }): Promise<{
  accessToken: string;
  refreshToken: string;
  expiresIn: number;
  scope: string;
 ...
 * verification.
 */
export const spotifyRedirect = onRequest({ secrets: [SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET] }, (req, res) => {
...
export const spotifyAuthorizeCodeGrant = onCall(
  ...
  async (request) => {
    ...
    info("received access token:", accessToken);
...
export const spotifyRefreshToken = onCall({ secrets: [SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET] }, async (request) => {
  const spotifyClient = new SpotifyWebApi({
    clientId: SPOTIFY_CLIENT_ID.value(),
    clientSecret: SPOTIFY_CLIENT_SECRET.value(),
    redirectUri: "https://app.tapped.ai/spotify",
...
export const getArtistBySpotifyId = onCall({ secrets: [SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET] }, async (request) => {
