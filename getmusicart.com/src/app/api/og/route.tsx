...
export async function GET(req: Request) {
  ...
  const height = parseInt(url.searchParams.get("height") ?? "1024", 10);
  const width = parseInt(url.searchParams.get("width") ?? "1024", 10);

  console.log({ imageUri, explicitContent });

  if (imageUri === undefined || imageUri === null) {
    return Response.json({ error: "no image uri provided" }, { status: 400 });
  }
  ...
  return new ImageResponse(
    ...
    >
      <img src={imageUri} alt="generated" />
      {explicitContent && (
        <img
